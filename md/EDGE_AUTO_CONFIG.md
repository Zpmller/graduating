...
当前 AI Edge 进程只管理一个视频源：主窗口内只有一个 `VideoCapture`、一个检测循环和一个推流器；本地控制服务也只维护一个全局推流状态。因此一个 AI Edge 进程不适合同时处理多台摄像头。

多设备部署建议：

- 每台摄像头运行一个 AI Edge 进程，并通过 `EDGE_HOST` 或 `DEVICE_TOKEN` 绑定到对应设备。
- 或后续重构为每个设备独立采集、检测、推流和状态管理的多路架构。
